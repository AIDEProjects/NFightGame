...
- [ ] 渲染前后关系问题: 设计一个渲染层组件来控制

### 内容新增- [ ] 滑铲
- [ ] 增加棍子武器：平A4连，一个小技能
- [ ] 做一个关卡：一个过关传送门，一个墙贴图，一种地面贴图，一种回血道具，一种伤害陷阱，选关界面，结算界面(死亡或胜利)
  - [ ]增加一个ai怪, 且每死亡一个几秒后会再次刷出一个更强的
- [ ] 背景改为灰度, 随时间改变颜色, 日升月落, 天空飘云, 下雨
- [ ] 增加游戏各个界面
...
- [x] 修正精灵默认原点为中心而不是左下角
- [x] 增加下落帧
- [x] component的toString改为他的gobject的name方便调试

### 0.12.0
- [x] 增加一个GSplitPanel在屏幕右边用于绘制显示一些实体状态数据
- [x] 修复检查器挡住触控问题: 将gui放到左侧空白table中
- [x] 增加检查器分割条宽度： 配置了skin
- [x] 检查器增加顶部切换栏， 切换检查器页与功能页
- [x] 功能页设置开关Gizmos绘制按钮
- [x] 将ui分辨率调到1152x540， 游戏世界1728x810
- [x] 增加直接点分割条切换检查器页面展开收起方式
#### 0.12.1
- [x] 死亡状态的实体不应启用碰撞器
- [x] 增加死亡状态判断方法
- [x] 给dummy也加上respawn
